...
const SaveAudience = ({ handleisSaveAudience }) => {
    ...
    const handleSaveSegment = async () => {
        if (segmentName === "" || newSchemas.length === 0) {
            alert("Please fill the required fields")
        } else {
            try {
                const data = {
                    segmentName,
                    schemas: newSchemas.map((schema) => ({ [schema.value]: schema.label })),
                };
                console.log('Sending data to server:', data);
                await axios.post("https://webhook.site/f9ea9f68-5efd-4883-b56d-e7317f7b7282", data)

            } catch (error) {
                console.log(error);
            }

            // setting the values to same as earlier
            setSegmentName('');
            setAvailableSchemas([
                { label: 'First Name', value: 'first_name', type: "user" },
                { label: 'Last Name', value: 'last_name', type: "user" },
    ...
    const handleAddNewSchema = () => {
        setNewSchemas([...newSchemas, { label: 'New Schema', value: 'new_schema', type: 'user' }]);
    };
    ...
    return (
        <div className='Main_container__inner'>
            <header><MdArrowBackIos size={30} />Saving Audience</header>
            ...
            <div className='SaveAudience_container'>
                ...
                {newSchemas.map((schema, index) => (
                    <div key={index} className='SaveAudience_container__inputs'>
                        <span>{schema.type === "user" ? <span>🟢</span> : <span>🔴</span>}</span>
                        <select value={schema.value} onChange={(e) => handleSchemaChange(e, index)} className='SaveAudience_container__schemaInputs'>
                        ...
                        <span className='deleteIcon' onClick={() => handleDeleteSchema(index)}><FcMinus size={25} /></span>
                    </div>
                ))}
                <p className='SaveAudience_container__addSchema'>+<span onClick={handleAddNewSchema}>
                    Add new schema
                </span></p>
            </div>

            <footer>
                <button className='SaveAudience_container__button' onClick={handleSaveSegment}>Save the Segment</button>
